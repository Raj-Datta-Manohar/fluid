...
func Start(ctx context.Context, logger *slog.Logger, cc consensus.ConsensusClient, addr string, createApp CreateAppFunc) func(context.Context) error {
	...
	mux.HandleFunc("/v1/apps", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := createApp(cctx, req.AppID, req.IP); err != nil {
			if _, notLeader := consensus.IsNotLeader(err); notLeader {
